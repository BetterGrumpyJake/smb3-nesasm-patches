...
ObjectGroup00_NormalJumpTable:
	...
	.word ObjNorm_HoldNote	; Object $10
...
ObjectGroup00_Attributes:
	.byte OA1_PAL0 | OA1_HEIGHT16 | OA1_WIDTH16	; Object $00
...
ObjNorm_HoldNote:
	...
	LDA <Objects_Var5,X			;var5 is the bump frame counter, if not bouncing on then do world interaction stuff
	BEQ HoldNote_World
	
	;bouncing effect
	;Bouncer_PUpVel:	.byte $00, -$40, -$40, -$30, -$20, -$10, $00, $10, $20, $30, $40
	TAY							;Y is now <Objects_Var5, starts at 8
	LDA Bouncer_PUpVel,Y		;A = var 5 index
	STA <Player_YVel			;store Bouncer_PUpVel value into players Y vel to match bouncing with noteblock
	
	CMP #HOLDNOTE_MAXBOB		;check if the index value is negative and if it is, just use -$10
	BPL HoldNote_BobOK
	LDA #HOLDNOTE_MAXBOB

HoldNote_BobOK:
	STA <Objects_YVel,X			;store the Bouncer_PUpVel into noteblocks Y vel, clamped at -$10
	JSR Object_ApplyYVel		;apply that velocity
	
	LDA <Pad_Input				;if pressed jump, set timer2 to 11 for high bouncing
	AND #PAD_A
	BEQ HoldNote_NoHigh
...
HoldNote_NoHigh:
	DEC <Objects_Var5,X			;decrement var5 through the noteblock bob
	BNE HoldNote_Ret2			;RTS if > 0, otherwise bob is done

	;noteblock bob done
	LDA #-$38					;set marios velocity to normal noteblock bounce
	
	LDY Objects_Timer2,X		;if mario didnt push A to bounce, keep -$38
	BEQ HoldNote_SetCap
	
	LDA Sound_QPlayer			;only play sound on high bounce
	ORA #SND_PLAYERJUMP
	STA Sound_QPlayer
...
;world interaction, taken from bobomb
HoldNote_World:
	JSR Object_Move
...
HoldNote_FloatTick:
	LDA Objects_Var12,X			;if counter already expired, do hit test
	BEQ HoldNote_HitTest
	DEC Objects_Var12,X			;otherwise decrement timer, if it's nonzero do hittest
	BNE HoldNote_HitTest
	LDA #$01
	STA Player_mGoomba			;if timer is 0 this frame, set flag. this cuts the upward velocity in prg8 short. it then gets cleared at PRG008_ACC8

HoldNote_HitTest:
	JSR Object_HitTest			;is mario touching the noteblock, carry set=collision
	BCC HoldNote_Ret
	LDA Objects_PlayerHitStat,X	;did mario land on it from above
	AND #$01
	BNE HoldNote_OnTop			;if so, do on top of noteblock stuff
...
HoldNote_OnTop:
	...
	BMI HoldNote_Ret
	LDA #HOLDNOTE_BUMPLENGTH	;landed on top, start bump frame timer
	STA <Objects_Var5,X
	STA <Player_InAir			;bumplength is nonzero, mark player as in air
	LDA #$00
	STA Objects_Timer2,X		;clear high-bounce gate
	RTS
